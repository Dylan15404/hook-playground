...
use crate::utils::{bytes_to_ascii, get_process_handle, windows_i8_path_to_filename, windows_i8_path_to_string};
...
pub unsafe fn load_modules(&mut self, pid: u32){
    ...
    loop {
        ...
        //make module object
        let mut this_module = module::new(module_base, module_size as u64, index, path);

        this_module.dirty_data = Some(buffer);
        this_module.valid = Some(total_bytes_read > 0);
        this_module.pages_valid = pages_valid;

        let name = windows_i8_path_to_filename(path);
        this_module.name = Some(name);
    ...
    pub fn read_modules(&mut self) -> Result<()> {
        for module in &mut self.modules {
            ...
            println!("module path: {:?}", windows_i8_path_to_string(module.file_path));
            if module.valid.unwrap() {
                match module.read_header() {
                    Ok(()) => {
                        for entry in &module.iat_dict {
                            println!("module: {}, va {}, from {}", entry.0, entry.1.0, entry.1.1);
                        }
                    }
                    Err(e) => {
